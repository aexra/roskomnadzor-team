...
def analyze_metrics(metrics):
    ...
    if 'read_bytes' in io_data and 'write_bytes' in io_data:
        ...
        if io_read_gb > 1 or io_write_gb > 1:
            warning("Проблема: Bottleneck ввода-вывода обнаружен. Рекомендации: Установите быстрый планировщик диска: echo mq-deadline > /sys/block/<disk>/queue/scheduler. Увеличьте буфер чтения: blockdev --setra 8192 /dev/<disk>. Оптимизируйте запись на диск: echo \"vm.dirty_ratio=20\" >> /etc/sysctl.conf && sysctl -p. Используйте SSD; следите с iostat или iotop.")
        else:
            info("Проблема: Bottleneck ввода-вывода не обнаружен.")

if __name__ == "__main__":
    if len(sys.argv) != 2:
        error("Использование: python io_bottleneck.py <PID или имя процесса>")
        sys.exit(1)
